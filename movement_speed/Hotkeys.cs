...
class Hotkeys {
    ...
    public const int HOTKEY_SPRINT = 1;
    private static Dictionary<int, List<KeyCode>> m_hotkeys = null;
    public static Dictionary<int, List<KeyCode>> HotKeys => m_hotkeys;

    public static void load() {
        m_hotkeys = new Dictionary<int, List<KeyCode>>();
        set_hotkey("", HOTKEY_MODIFIER);
        set_hotkey(Settings.m_hotkey_sprint.Value, HOTKEY_SPRINT);
    }
    ...
    public static bool is_hotkey_down(int key_index, bool use_GetKeyDown = true) {
        foreach (KeyCode key in m_hotkeys[key_index]) {
            if ((use_GetKeyDown ? Input.GetKeyDown(key) : Input.GetKey(key))) {
                return true;
            }
        }
        return false;
